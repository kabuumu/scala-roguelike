...
object Movement {
  def moveEvent(id: String, dir: Direction) = Event {
    case (e: Mover) if e.id == id =>
      ...
      val f = Event { case e: Mover if e.id == id => (Iterable(e.pos(_ => newPos).asInstanceOf[Entity with Mover]), Nil) }

      (Iterable(e.facing(dir).asInstanceOf[Entity with Mover]), Seq(withCheckCollision(newPos, e.pos, e, f)))
  }
