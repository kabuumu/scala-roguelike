...
class GameStateSpec extends WordSpec with Matchers with TestFixture {
  "update" should {
    ...
    "apply events that use the ComponentQuery class" in {
      val testComponent = IntComponent(0)
      val state = GameState(Seq(Entity(IntComponent(0))))
      val testEvent = event.when[IntComponent] eq testComponent update increment

      val expectedN = increment(testComponent)
      val expectedEntity = Entity(expectedN)
